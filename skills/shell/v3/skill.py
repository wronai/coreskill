...
class ShellSkill:
    ...
    def execute(self, params: dict) -> dict:
        ...
        if not command:
            return {"success": False, "error": "No command provided", "spoken": "No command provided"}

        # Safety check
        cmd_lower = command.lower().strip()
        for blocked in BLOCKED_COMMANDS:
            if blocked in cmd_lower:
                return {"success": False, "error": f"Blocked dangerous command: {command}", "spoken": f"Blocked dangerous command: {command}"}
        
        # Check for dangerous prefixes as well
        for prefix in DANGEROUS_PREFIXES:
            if cmd_lower.startswith(prefix):
                 return {"success": False, "error": f"Blocked dangerous command prefix: {command}", "spoken": f"Blocked dangerous command prefix: {command}"}
        ...
        try:
            ...
            if interactive:
                ...
                try:
                    ...
                    return {"success": False, "command": command,
                            "error": f"Command timed out after {timeout}s", "spoken": f"Command timed out after {timeout}s"}
                return {
                    ...
                    "stderr": "",
                    "spoken": "Command executed interactively"
                }
            else:
                ...
                try:
                    ...
                    proc.kill()
                    return {"success": False, "command": command,
                            "error": f"Command process did not terminate cleanly after {timeout}s", "spoken": f"Command process did not terminate cleanly"}
                ...
                if proc.returncode == 0:
                    spoken = "Command executed successfully"
                    if stdout_lines:
                        spoken = f"Command executed successfully. Output: {'; '.join(stdout_lines[:3])}"
                else:
                    error_message = stderr_log[:500] if stderr_log else f"Command failed with exit code {proc.returncode}"
                    spoken = f"Command failed with exit code {proc.returncode}"
            ...
            return {"success": False, "command": command, "error": f"Command not found: {command.split()[0]}", "spoken": f"Command not found: {command.split()[0]}"}
        except Exception as e:
            return {"success": False, "command": command, "error": str(e), "spoken": f"Error executing command: {str(e)}"}
