...
class ContentSearchSkill:
    ...
    def execute(self, params: dict) -> dict:
        try:
            ...
            # Use web_search via subprocess if available (assuming it's installed as a skill)
            # Since we can't use pip packages, we'll try to call web_search skill via CLI if available
            # Otherwise, fall back to basic web search using urllib and duckduckgo
            try:
                # Try to use web_search skill if it's available as a module
                import web_search
                ws_result = web_search.execute({'text': query})
                if ws_result.get('success') and ws_result.get('results'):
                    return {
                        'success': True,
                        'results': ws_result['results'][:3],  # Return top 3 results
                        'message': f"Found {len(ws_result['results'])} results using web_search"
                    }
            except (ImportError, AttributeError):
                pass  # Fall back to direct web search
