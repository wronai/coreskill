    ...
    def execute(self, params: dict) -> dict:
        try:
            ...
            if re.search(r'[+\-*/]{2,}', expression):
                return {'success': False, 'error': 'Nieprawidłowe rozmieszczenie operatorów.'}
            # Disallow operators at the start or end
            if re.search(r'^[*/]', expression) or re.search(r'[+\-*/]$', expression):
                return {'success': False, 'error': 'Operator na początku lub końcu wyrażenia.'}
            # Disallow empty parentheses
            if re.search(r'\(\)', expression):
                return {'success': False, 'error': 'Puste nawiasy są niedozwolone.'}
