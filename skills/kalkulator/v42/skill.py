...
class Kalkulator:
    ...
    def _extract_expression(self, text: str) -> str | None:
        # Prosta próba wyodrębnienia wyrażenia matematycznego
        # Zakłada, że wyrażenie jest po słowie "oblicz" lub "ile to"
        match = re.search(r'(?:oblicz|ile to)\s+(.+)', text, re.IGNORECASE)
        if match:
            return match.group(1).strip()
        
        # Alternatywnie, jeśli tekst sam w sobie jest wyrażeniem
        # (np. "2+2") - wymaga ostrożności
    ...
    def _calculate(self, expression: str) -> float:
        # Bezpieczne wykonanie obliczeń
        # Używamy eval z ograniczeniami lub zewnętrznego narzędzia, jeśli dostępne
        # Tutaj użyjemy prostego eval, ale w bardziej złożonych scenariuszach
        # należałoby rozważyć bezpieczniejsze metody lub biblioteki.
        # Ponieważ nie możemy używać pip, eval jest jedyną opcją w stdlib
        # dla prostych obliczeń.
        
        # Ograniczenie znaków dozwolonych w wyrażeniu, aby uniknąć złośliwego kodu
...
if __name__ == '__main__':
    ...
    test_cases = [
        ...
        {'text': '2**3', 'expected_success': False, 'expected_error': 'Niedozwolone znaki w wyrażeniu'} # ** nie jest dozwolone
    ]
