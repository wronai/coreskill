...
class Kalkulator:
    ...
    def _extract_expression(self, text: str) -> str:
        ...
        # Zaktualizowany regex, aby lepiej obsługiwać nawiasy i ciągi operacji
        match = re.search(r'([\d\.\s\(\)\+\-\*\/\%]+)', text)
        if match:
            expression = match.group(1).strip()
            # Dodatkowe sprawdzenie, czy wyodrębnione wyrażenie faktycznie wygląda jak matematyczne
            if re.search(r'\d', expression): # Upewnij się, że jest jakaś liczba
                return expression
        return ""
    ...
    def _calculate(self, expression: str) -> float:
        ...
        try:
            # Dodajemy obsługę potęgowania (**) jeśli jest potrzebna, ale na razie zostawiamy podstawowe operatory
            # Upewniamy się, że nie ma pustych wyrażeń lub tylko operatorów
            if not expression or not re.search(r'\d', expression):
                raise ValueError("Puste lub nieprawidłowe wyrażenie.")

            # Sprawdzenie czy wyrażenie nie kończy się operatorem
            if expression[-1] in '+-*/%':
...
if __name__ == '__main__':
    test_cases = [
        ...
        {'text': 'Oblicz 100 / (5 * 2) + 5 - 2 * 3 + 4 / (2 * 2) - 1 + 2 * 3 / (3 * 3) + 1 -