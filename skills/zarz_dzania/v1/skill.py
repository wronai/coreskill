...
class ZarzadzaniaSkill:
    ...
    def copy_file(self, text: str) -> dict:
        ...
        if not match:
            match = re.search(r'(kopiuj|cp)\s+(.+?)\s+(.+)', text) # Alternative format without 'do'
            if not match:
...
if __name__ == '__main__':
    ...
    print(execute({'text': 'listuj /tmp'}))
    print(execute({'text': 'ls'}))
    ...
    print(execute({'text': 'kopiuj test_source.txt /tmp/'}))
    print(execute({'text': 'kopiuj non_existent_file.txt test_dest.txt'}))
    print(execute({'text': 'kopiuj test_source.txt'})) # Invalid format

    print("\n--- Test Info o pliku ---")
    print(execute({'text': 'info o pliku test_source.txt'}))
    print(execute({'text': 'stat test_destination.txt'}))
