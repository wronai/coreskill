...
def health_check() -> dict:
    try:
        subprocess.run(['uname', '-a'], check=True, capture_output=True, timeout=5)
        return {'status': 'ok'}
    ...
    def execute(self, params: dict) -> dict:
        ...
        try:
            if 'info' in text or 'pokaż' in text or 'system' in text:
                ...
                spoken_text = "Here is the system information."
                return {'success': True, 'data': system_info, 'spoken': spoken_text}
            ...
            if file_list_match:
                ...
                spoken_text = f"Here are the files in {directory}: {', '.join(files[:5])}." + (" and more." if len(files) > 5 else "")
                return {'success': True, 'data': {'directory': directory, 'files': files}, 'spoken': spoken_text}

            # Handle Polish "jaka jest w latach" query for age calculation (if needed)
            age_match = re.search(r'jaka jest w latach', text)
            if age_match:
                return {'success': True, 'data': {'message': 'Age calculation not implemented.'}, 'spoken': 'Age calculation is not implemented yet.'}

            return {'success': False, 'message': "Unknown command. Try asking for 'system info' or 'list files in <directory>'.", 'spoken': "Unknown command. Try asking for 'system info' or 'list files in <directory>'."}
        except Exception as e:
...
if __name__ == '__main__':
    ...
    # Test case 7: Age query
    test_params_age = {'text': 'jaka jest w latach'}
    result_age = execute(test_params_age)
    print(f"Test Case 7 (Age query): {result_age}")