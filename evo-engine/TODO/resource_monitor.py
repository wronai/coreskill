#!/usr/bin/env python3
"""
resource_monitor.py — System resource detection for skill provider selection.

Usage:
    mon = ResourceMonitor()
    snap = mon.snapshot()
    can, reason = mon.can_run({"gpu": True, "min_ram_mb": 4096})
"""
import os
import shutil
import subprocess
...
if __name__ == "__main__":
    ...
    mon = ResourceMonitor()
    snap = mon.snapshot()
    # Don't dump all packages
    snap["python_packages"] = f"({len(snap['python_packages'])} packages)"
    ...
    tests = [
        ...
        {"system_packages": ["nvidia-smi"]},
    ]
    for req in tests:
        ok, reason = mon.can_run(req)
        print(f"  {req} -> {'OK' if ok else 'FAIL'}: {reason}")
