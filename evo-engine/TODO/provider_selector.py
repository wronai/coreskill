#!/usr/bin/env python3
"""
provider_selector.py — Resource-aware skill provider selection.
...
Example:
    selector = ProviderSelector(resource_monitor)
    provider = selector.select("tts")                      # -> "espeak" or "coqui"
    provider = selector.select("tts", prefer="quality")    # -> "coqui" if GPU
    provider = selector.select("tts", prefer="speed")      # -> "espeak"
    provider = selector.select("tts", force="pyttsx3")     # -> "pyttsx3"
"""
import json
...
class ProviderSelector:
    ...
    def __init__(self, skills_dir: Path, resource_monitor=None):
...
EXAMPLE_ESPEAK_META = {
    ...
    "quality_score": 3,
    "requirements": {
        "system_packages": ["espeak"]
    },
    "fallback_to": None,
    "tags": ["offline", "fast", "low-quality"],
    "auto_install": True
}
...
if __name__ == "__main__":
    print("=== Example manifest.json (TTS) ===")
    print(json.dumps(EXAMPLE_TTS_MANIFEST, indent=2))
    print("\n=== Example meta.json (espeak) ===")
    print(json.dumps(EXAMPLE_ESPEAK_META, indent=2))
    print("\n=== Example meta.json (coqui) ===")
    print(json.dumps(EXAMPLE_COQUI_META, indent=2))
