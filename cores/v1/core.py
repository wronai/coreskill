...
def _flush_stdin():
    ...
    Called after voice loop exits to clean up any stray characters that may have been
    generated by terminal redraws or subprocess interruptions (e.g., Ctrl+T during arecord).
    """
    import sys
    import select
    import termios
    
    # Try to flush any pending input using select (non-blocking check)
    try:
        # On Unix-like systems, use select to check if there's input waiting
        if hasattr(select, 'select'):
            # Give terminal a moment to settle after voice loop
            import time
            time.sleep(0.1)
        ...
        # If any terminal manipulation fails, just continue silently
        pass
